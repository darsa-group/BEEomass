...
def parse_label(image_path, label_pattern, min_conf=0.2):
    import easyocr
...
def parse_barcode( barcode_path: Path, label_pattern) -> Dict[str, Any]:
    ...
    try:
        ...
        if enforce_barcode:
            ...
            if str(barcode_path).endswith("thumb.jpg"):
                ...
                out = parse_label(barcode_path.with_suffix("").with_suffix(".jpg"), label_pattern=label_pattern)
...
def segment_dir(fb_weights, in_dir_root, out_dir_root, image_scale, force, label_pattern):
    ...
    for f in glob.glob(str(Path(in_dir_root) / "**" / "*.json"), recursive=True):
        ...
        try:
            ...
            label = parse_barcode( thumbnail, label_pattern=label_pattern)
