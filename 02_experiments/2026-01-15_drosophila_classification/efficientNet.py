...
def show_batch_cv2(
    ...
    print(tile.shape)
    cv2.imwrite("/tmp/droso_aug.png", tile)
...
class MultiTaskOutput:
    ...
    species_logits: torch.Tensor   # [B, 5]


class EfficientNetMultiTask(nn.Module):
    """
    EfficientNet backbone with two heads:
      - sex: 2 classes
      - species: N classes
    """
    def __init__(
        ...
        nn.init.normal_(self.sex_head.weight, mean=0.0, std=0.01)
        nn.init.zeros_(self.sex_head.bias)
        nn.init.normal_(self.species_head.weight, mean=0.0, std=0.01)
        nn.init.zeros_(self.species_head.bias)
...
def multitask_loss(
    ...
    loss = w_sex * loss_sex + w_species * loss_species
    return loss, {
...
def run_eval_epoch(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    for images, sex, species in loader:
        ...
        both_correct += ((ps == sex) & (pp == species)).sum().item()
...
def save_predictions_csv(
    ...
    if not hasattr(ds, "df"):
        raise ValueError("Dataset must have attribute .df with IMAGE_FILENAME/species/sex columns.")
    ...
    # If your loader uses a different batch_size later, don't rely on loader.batch_size for indexing.
    # We'll advance with a running offset.
    for images, sex, species in loader:
        bs = images.size(0)
        ...
        # pull corresponding GT strings + filename from dataset.df
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (images, sex, species) in enumerate(train_loader, start=1):
            images = images.to(device, non_blocking=True)
            sex = sex.to(device, non_blocking=True)
            species = species.to(device, non_blocking=True)
            ...
            with torch.cuda.amp.autocast(enabled=scaler.is_enabled()):
                out = model(images)
                loss, m = multitask_loss(out, sex, species, w_sex=w_sex, w_species=w_species)
            ...
            if step == 1 or step % 20 == 0 or step == len(train_loader):
                ...
                print(
                    ...
                    f"(sex={loss_sex_sum/max(n,1):.4f}, sp={loss_species_sum/max(n,1):.4f}) "
        ...
        print(
            ...
            f"train loss={train_metrics['loss']:.4f} sex_acc={train_metrics['sex_acc']:.3f} "
            f"sp_acc={train_metrics['species_acc']:.3f} both={train_metrics['both_acc']:.3f} | "
            f"val loss={val_metrics['loss']:.4f} sex_acc={val_metrics['sex_acc']:.3f} "
            f"sp_acc={val_metrics['species_acc']:.3f} both={val_metrics['both_acc']:.3f}"
        )
...
if __name__ == "__main__":
    ...
    out_dir = "checkpoints_droso_effnet"
    epochs = 50
    ...
    train_tf = T.Compose([
        ...
        T.RandomResizedCrop(
            image_size,
            scale=(0.6, 1.0),  # how much of the image area to keep
